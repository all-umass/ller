...
def ller(
        X, Y, n_neighbors, n_components, reg=1e-3, eigen_solver='auto', tol=1e-6,
        max_iter=100, random_state=None, mu=0.5, gamma=None):
    ...
    L /= np.abs(L).max()
    omega = M+mu*L
    embedding,lle_error = null_space(omega, n_components, k_skip=1, eigen_solver=eigen_solver,
...
class LLER(LocallyLinearEmbedding):
    ...
    def fit_transform(self, X, Y):
        ...
        self.nbrs_.fit(X)
        self.embedding_, self.lle_error_, self.ller_error_ = \
            ller(self.nbrs_, Y, self.n_neighbors, self.n_components,
